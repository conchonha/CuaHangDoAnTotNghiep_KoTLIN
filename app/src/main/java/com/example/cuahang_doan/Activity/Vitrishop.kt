...
class Vitrishop : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val cuahangonline = LatLng(15.972540, 108.249584)
        mMap!!.addMarker(
            MarkerOptions().position(cuahangonline).title("Truuong Cao Dang CNTT-DHDN")
                .snippet("Duong Nam Ky Khoi Nghia").icon(BitmapDescriptorFactory.defaultMarker())
        )
        mMap!!.mapType = GoogleMap.MAP_TYPE_SATELLITE
        val cameraPosition = CameraPosition.Builder().target(cuahangonline).zoom(15f).build()
